(() => {
	...
	const repeaterSelector = '.acf-field-repeater[data-key="field_proof_logos_logos"]';
	...
	const createControl = (label, shouldCollapse) => {
		...
		button.type = 'button';
		button.className = 'proof-logos-editor__control';
	...
	const enhanceRepeater = (field) => {
		if (field.dataset.proofLogosControlsReady) {
			return;
		}
		...
		controls.className = 'proof-logos-editor__controls';
		controls.append(
		...
		field.dataset.proofLogosControlsReady = 'true';
	};
