(() => {
	...
	const repeaterSelector = '.acf-field-repeater[data-key="field_impact_metrics_metrics"]';
	...
	const createControl = (label, shouldCollapse) => {
		...
		button.type = 'button';
		button.className = 'impact-metrics-editor__control';
	...
	const enhanceRepeater = (field) => {
		if (field.dataset.impactMetricsControlsReady) {
			return;
		}
		...
		controls.className = 'impact-metrics-editor__controls';
		controls.append(
		...
		field.dataset.impactMetricsControlsReady = 'true';
	};
