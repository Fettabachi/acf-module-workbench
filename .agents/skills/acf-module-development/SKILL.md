...
8. Complete mobile-first responsive behavior across narrow, intermediate, and wide layouts. When a supplied design gives different mobile and desktop typography for the same heading, record both size and line-height endpoints and the intended interpolation range in the module contract. Prefer rem-bounded fluid sizing between them unless the design calls for a deliberate breakpoint change; do not require fluid sizing for every heading. Use Grid or Flexbox for layout and JavaScript only for required behavior.
